...
## Живой пример: 5 вопросов, один прогон поиска

Дальше все определения будем проверять на одной и той же таблице, чтобы формулы не висели в воздухе. Взяли 5 знакомых вопросов из журнала случаев и прогнали поиск с `k=5` (возвращаем пять кандидатов на вопрос, а не один):
...
Это и есть сырые данные. Всё, что ниже, — просто разные способы посчитать по ним одно число.

## Две метрики

> **Hit@k** — доля вопросов, для которых нужный кусок попал в первые k найденных. Отвечает на вопрос «нашли ли вообще».

Считаем Hit@3 по таблице выше: смотрим, у кого место **1, 2 или 3**.
...
Считаем MRR по той же таблице — для каждого вопроса берём 1, поделённую на место (для «не нашёлся» — просто 0, делить не на что):
...
Разница видна сразу: Hit@3 спрашивает только «да/нет», а MRR наказывает и за то, что нужный кусок нашёлся, но не на первом месте.
