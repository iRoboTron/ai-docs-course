...
Открой `app/rag.py` в [ai-docs-course-bot](https://github.com/iRoboTron/ai-docs-course-bot),
найди `proverit_citatu` — строку `if citata in normalizovat(kusok["tekst"]):`.
Замени `in` на `==` (было «цитата есть внутри куска», станет «цитата равна
всему куску»).

**Прежде чем запускать: как думаешь, какой тест из `tests/test_rag_checks.py`
теперь упадёт — и почему именно он, а не остальные?**
...
Упадёт `test_proverit_citatu_naxodit_dosolvnuyu_citatu`: цитата в тесте —
«мы даём гарантию 12 месяцев», а весь кусок — «На выполненные работы мы даём
гарантию 12 месяцев.». Это подстрока, а не то же самое целиком.

Дело не в опечатке. Модель по инструкции цитирует **фрагмент**, на котором
основан ответ, а не пересказывает кусок документа целиком слово в слово. Проверь
`in`, а не `==` — иначе такая проверка отвергнет вообще все настоящие ответы,
а не только выдуманные. Верни `in` обратно, прежде чем читать дальше.
