...
`count_tokens_approximately` — это быстрая оценка, а не точный токенайзер
модели (настоящее число будет чуть другим), но для решения «сколько сообщений
влезет» точность до токена и не нужна.

| Сообщение | Примерно токенов |
|---|---|
| System: «Ты помощник сервисного центра…» | 15 |
| Human: «Во сколько вы закрываетесь в субботу?» | 14 |
| AI: «В субботу мы работаем до 17:00.» | 13 |
| Human: «А в воскресенье?» | 8 |
| AI: «В воскресенье мы не работаем.» | 13 |
| Human: «Какая у вас гарантия на ремонт?» | 12 |

При `strategy="last"` сборка идёт **с конца**: системное сообщение берётся
всегда (`include_system=True`), а дальше добавляются реплики от самой свежей
к самой старой, пока хватает бюджета.

| Шаг | Добавили | Занято токенов | Осталось от 50 |
|---|---|---|---|
| 1 | System (15) | 15 | 35 |
| 2 | «Какая у вас гарантия…» (12) | 27 | 23 |
| 3 | «В воскресенье мы не работаем.» (13) | 40 | 10 |
| 4 | «А в воскресенье?» (8) | 48 | 2 |
| 5 | «В субботу мы работаем до 17:00.» (13) | **не влезает** (48+13=61 > 50) | — остановка |

Как только следующее сообщение не влезает в остаток бюджета — сборка
останавливается, и всё, что старше, отбрасывается целиком (весь обмен про
субботу, а не половина). Результат:
...
Системное правило (`include_system=True`) остаётся всегда, вне очереди на
обрезку: без него бот забудет, кто он, после первой же обрезки.
