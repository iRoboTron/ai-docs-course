...
## Готовые фреймворки для оценки

Мы собрали измеритель сами: эталонный набор, проверки кодом, судья. На работе за это чаще отвечают готовые программы.

> **Фреймворк для оценки** (по-английски *evaluation framework*) — готовый инструмент, который прогоняет эталонный набор через ИИ-систему, считает метрики и отдаёт число.

| Фреймворк | Для чего | Похож на то, что мы сделали руками |
|---|---|---|
| **RAGAS** | метрики правдивости и полноты поиска именно для RAG | эталонный набор + судья |
| **DeepEval** | набор метрик и проверок, гоняется как тесты | судья + `pytest` из наших трёх файлов |
| **promptfoo** | таблица «до» и «после»: ответы сначала и после правки рядом | наш журнал решений `decisions.md` |
| **LangSmith** | журнал реальных запросов и сравнение версий | журнал запросов и `cases.jsonl` |

Два следствия, ради которых всё это поминают.

Первое: фреймворк не избавляет от эталонного набора — **он его ест**. Число из RAGAS не появится из воздуха: ему нужны вопросы, ответы бота, найденные куски и правильные ответы. Всё то, что мы собирали весь модуль.

Второе: метрики внутри фреймворка считает **та же модель-судья** со всеми слабостями из шага 7. Красивое число «0.96» — не подтверждённый факт, а мнение модели. Сверка не отменяется, просто переносится на уровень фреймворка.
...
**Готовый фреймворк на том же наборе.** Тот же бот и те же вопросы прогнали через RAGAS — прогон 19 сентября 2026 на `qwen3.7-flash`, 7 вопросов с ответами (вопросу «оплатить картой?» в фреймворке места нет: метрика для отказа не задумана). Средняя **правдивость** (*faithfulness*, каждое утверждение ответа подтверждается найденным) — 0.96, средняя **полнота поиска** (*context_recall*, правильный ответ покрывается найденным) — 0.86. И снова всё снижает один вопрос — «гарантия»: полнота 0.00 (поиск не принёс кусок про 12 месяцев), правдивость 0.75 (одно утверждение ответа осталось без подтверждения). Тот же провал, что нашёл наш код, — только теперь число от готового инструмента. И его стоит перепроверить: метрику считает та же модель-судья.
...
- **Фреймворки для оценки** (RAGAS и другие) считают те же метрики за нас, но эталонный набор и сверку чисел не отменяют: внутри метрик та же модель-судья.
