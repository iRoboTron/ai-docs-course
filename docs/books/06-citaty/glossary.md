...
## Фреймворк для оценки (evaluation framework)

> **Фреймворк для оценки** — готовый инструмент, который прогоняет эталонный набор через ИИ-систему, считает метрики и отдаёт число.

Из [урока 2](chapter-02.md). Для RAG — фреймворк RAGAS с метриками правдивости (faithfulness) и полноты поиска (context_recall). Фреймворк не отменяет эталонный набор и сверку: метрики считает модель-судья.

```mermaid
flowchart TD
    A["Вопросы + ответы + эталоны"] --> B["Фреймворк: счёт метрик"]
    B --> C["Числа: правдивость, полнота"]
    C --> D["Перепроверить: метрику считает модель"]
    style A fill:#2d2d2d,color:#fff
    style B fill:#1a5276,color:#fff
    style C fill:#1e8449,color:#fff
    style D fill:#7d6608,color:#fff
